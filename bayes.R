...
if (!exists("foo", mode = "function"))
  source("carregarDados.R")
if (!exists("foo", mode = "function"))
  source("avaliarDados.R")

stringQueryCountFUNCTIONS_derby <-
  "SELECT COUNT(*) FROM software.FUNCTIONS_derby;"
stringQueryFUNCTIONS_derby <-
  "SELECT * FROM software.FUNCTIONS_derby;"

fNaiveBayes <- function(stringQueryFUNCTIONS, stringQueryCountFUNCTIONS) {
  listTrainTestAndLabels <-
    getTrainTestAndLabels(stringQueryFUNCTIONS,
                          stringQueryCountFUNCTIONS)
  ...
  classifier20 <- naiveBayes(train20, labelTrain20)
  predict20 <- predict(classifier20, test20)
  valuesCrossTableBayes20 <-
    getValuesCrossTable(labelTest20, predict20)
  fMeasureResult20 <-
    as.numeric(getPrecisonRecallAndFMeasure(valuesCrossTableBayes20)[3])
  
  classifier40 <- naiveBayes(train40, labelTrain40)
  predict40 <- predict(classifier40, test40)
  valuesCrossTableBayes40 <-
    getValuesCrossTable(labelTest40, predict40)
  fMeasureResult40 <-
    as.numeric(getPrecisonRecallAndFMeasure(valuesCrossTableBayes40)[3])
  
  classifier60 <- naiveBayes(train60, labelTrain60)
  predict60 <- predict(classifier60, test60)
  valuesCrossTableBayes60 <-
    getValuesCrossTable(labelTest60, predict60)
  fMeasureResult60 <-
    as.numeric(getPrecisonRecallAndFMeasure(valuesCrossTableBayes60)[3])
  
  classifier80 <- naiveBayes(train80, labelTrain80)
  predict80 <- predict(classifier80, test80)
  valuesCrossTableBayes80 <-
    getValuesCrossTable(labelTest80, predict80)
  fMeasureResult80 <-
    as.numeric(getPrecisonRecallAndFMeasure(valuesCrossTableBayes80)[3])
  
  
  classifier100 <- naiveBayes(train, labelTrain)
  predict100 <- predict(classifier100, test)
  valuesCrossTableBayes100 <-
    getValuesCrossTable(labelTest, predict100)
  fMeasureResult100 <-
    as.numeric(getPrecisonRecallAndFMeasure(valuesCrossTableBayes100)[3])
...
#fNaiveBayes(stringQueryFUNCTIONS_derby, stringQueryCountFUNCTIONS_derby)
