...
fRandomForest <- function(stringQueryFUNCTIONS, stringQueryCountFUNCTIONS){
...
set.seed(length(labelTrain))##Permite resultados reprodutíveis da próxima vez que rodar o código.

classifier20 <- randomForest(as.factor(X1.Affected) ~ ., data = train20)
...
classifier40 <- randomForest(as.factor(X1.Affected) ~ ., data = train40)
...
classifier60 <- randomForest(as.factor(X1.Affected) ~ ., data = train60)
...
classifier80 <- randomForest(as.factor(X1.Affected) ~ ., data = train80)
...
classifier100 <- randomForest(as.factor(X1.Affected) ~ ., data = train)
...
#a <- barplot(allFMeasures, names.arg=c("20", "40", "60", "80", "100"), main="F-measure Random Forest", xlab="Porcentagem do conjunto de teste", ylab ="F-measure")

return(allFMeasures)
}

#fRandomForest(stringQueryFUNCTIONS_derby, stringQueryCountFUNCTIONS_derby)
