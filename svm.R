...
fSvm <- function(stringQueryFUNCTIONS, stringQueryCountFUNCTIONS){
...
classifier20 <- ksvm(as.factor(X1.Affected) ~ ., data = train20, kernel = "vanilladot")
...
classifier40 <- ksvm(as.factor(X1.Affected) ~ ., data = train40, kernel = "vanilladot")
...
classifier60 <- ksvm(as.factor(X1.Affected) ~ ., data = train60, kernel = "vanilladot")
...
classifier80 <- ksvm(as.factor(X1.Affected) ~ ., data = train80, kernel = "vanilladot")
...
classifier100 <- ksvm(as.factor(X1.Affected) ~ ., data = train, kernel = "vanilladot")
...
#a <- barplot(allFMeasures, names.arg=c("20", "40", "60", "80", "100"), main="F-measure SVM", xlab="Porcentagem do conjunto de teste", ylab ="F-measure")

return(allFMeasures)
}

#fSvm(stringQueryFUNCTIONS_derby, stringQueryCountFUNCTIONS_derby)
